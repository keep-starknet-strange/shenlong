...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    /// Implementation of the LLVM IR conversion of `array_push<T>` operation.
    ///
    /// # Arguments
    ///
    /// * `libfunc_declaration` - The corelib function declaration of `array_push<T>`.
    ///
    /// # Error
    ///
    ...
    pub fn array_append(&self, libfunc_declaration: &LibfuncDeclaration) {
        ...
        let arg_type = match &libfunc_declaration.long_id.generic_args[0] {
            ...
            GenericArg::Type(ConcreteTypeId { id, debug_name: _ }) => *self.types_by_id.get(id).unwrap(),
