...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    ...
    pub fn process_funcs(&mut self) -> CompilerResult<()> {
        ...
        for func_declaration in self.program.funcs.iter() {
            ...
            // let start_block = self.context.append_basic_block(func, &format!("{func_name}_entry"));
            // let variable_map = HashMap::new();

            // self.builder.position_at_end(start_block);

            // // Loop through the arguments of the function. The variable id counter always starts from zero
            // for a // function.
            // // TODO Check if args always go in slot numbered by argument position
            // for (var_id, _) in args.iter().enumerate() {
            //     // Save the function arguments in the variables map to be able to use them in the function
            // body.     let variable = func.get_nth_param(var_id as u32).expect("Function should
            // have enough parameters");     variable_map.insert(var_id as u64, variable);
            // }
            ...
            // // process statements from the line stated in the function definition until the return
            // instruction. // ex: fib_caller::fib_caller::main@21() -> (Unit); the function main
            // starts at the statement 21. // self.process_statements_from(func,
            // func_declaration.entry_point.0, debug_function.scope)?;

            // // The function has been processed. Valid functions have always 1 block and atleast 1
            // instruction. // let first_instruction =
            // func.get_first_basic_block().unwrap().get_first_instruction().unwrap(); // for (i,
            // var) in func.get_param_iter().enumerate() { //     self.debug.insert_dbg_value(
            // //         var,
